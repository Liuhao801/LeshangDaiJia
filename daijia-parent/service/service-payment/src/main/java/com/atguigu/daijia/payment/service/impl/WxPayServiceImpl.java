...
public class WxPayServiceImpl implements WxPayService {
    ...
    public WxPrepayVo createWxPayment(PaymentInfoForm paymentInfoForm) {
        try {
            PaymentInfo paymentInfo = paymentInfoMapper
                    .selectOne(new LambdaQueryWrapper<PaymentInfo>().eq(PaymentInfo::getOrderNo, paymentInfoForm.getOrderNo()));
            if(null == paymentInfo) {
                paymentInfo = BeanUtil.copyProperties(paymentInfoForm, PaymentInfo.class);
            ...
            log.info("微信支付下单返回参数：{}", JSONUtil.toJsonStr(response));
            ...
            e.printStackTrace();
            throw new GuiguException(ResultCodeEnum.WX_PAY_ERROR);
        }
    }
    ...
    public Boolean queryPayStatus(String orderNo) {
        ...
        try {
            ...
            log.info(JSONUtil.toJsonStr(transaction));
    ...
    public void wxnotify(HttpServletRequest request) {
        ...
        log.info("成功解析：{}", JSONUtil.toJsonStr(transaction));
        if(null != transaction && transaction.getTradeState() == Transaction.TradeStateEnum.SUCCESS) {
            //5.处理支付业务
            this.handlePayment(transaction);
        }
    ...
    public void handlePayment(Transaction transaction) {
        PaymentInfo paymentInfo = paymentInfoMapper
                .selectOne(new LambdaQueryWrapper<PaymentInfo>().eq(PaymentInfo::getOrderNo, transaction.getOutTradeNo()));
        ...
        paymentInfo.setCallbackContent(JSONUtil.toJsonStr(transaction));
